...
// getPrompt
// opens .connrc and prints the prompt
...
void getPrompt() {
    ...
    if ((shrc = fopen(".connrc", "r")) == NULL ) {
        
        printf("Error: .connrc could not be opened. Please make sure the file exists and is properly formatted. Exiting program.\n");    
        exit(1);
    }
...
// getFileLoc
// Opens .connrc, scans the lines after line 1 to find a path, and checks if the path is valid
...
void getFileLoc(char inString[]) {
    ...
    if ((shrc = fopen(".connrc", "r")) == NULL ) {
        printf("Error occurred when attempting to open .connrc.");
	...
	// in case .connrc doesn't have the / after the PATH.
...
int main() {
    ...
    while (exitCmd == 0) {
        ...
        // print the contents of .connrc, then gets input
        getPrompt();
